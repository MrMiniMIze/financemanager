...
import type { GetProfileContext } from '../services/profile-service';
...
const authContextPlugin: FastifyPluginAsync = async (fastify) => {
  ...
  fastify.decorate(
    ...
    function resolveProfileContext(request: FastifyRequest): GetProfileContext {
      const user = request.authUser;
      return user
        ? { email: user.email ?? null, timezone: user.timezone ?? null }
        : { email: null, timezone: null };
    },
  );
};
