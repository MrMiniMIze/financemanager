...
function computeOverview(goals: Goal[], fallback?: GoalsWorkspaceOverview): GoalsWorkspaceOverview {
  if (goals.length === 0) {
    return (
      fallback ?? {
        totalGoals: 0,
...
export function GoalsSection() {
  ...
  const overview = useMemo(
    () => computeOverview(goals, workspace?.overview),
    [goals, workspace?.overview],
  );
  ...
  const insights = useMemo(() => {
    ...
    return workspace.insights.filter(
      (insight) => !insight.goalId || insight.goalId === selectedGoal.id,
    );
        ...
        <Card className={styles.summaryCard}>
          ...
          <CardBody className={styles.summaryBody}>
            ...
            <div className={styles.summaryLabel}>
              of {formatCents(overview.totalTargetCents)} funded
            </div>
        ...
        <Card className={styles.summaryCard}>
          ...
          <CardBody className={styles.summaryBody}>
            <div className={styles.summaryMetric}>
              {formatCents(overview.monthlyCommitmentCents)}
            </div>
      ...
      <div className={styles.mainGrid}>
              ...
              return (
                ...
                >
                  ...
                  <div className={styles.goalStatusRow}>
                    <span
                      className={[styles.goalHealthTag, healthToneClass[goal.health]].join(' ')}
                    >
                      {healthLabels[goal.health]}
                    </span>
                    <span className={styles.goalTimelineLabel}>
                  ...
                  <div className={styles.goalMetaRow}>
                    <div>
                      ...
                      <span className={styles.goalMetaValue}>
                        {formatShortDate(goal.nextContributionDate)} â€¢{' '}
                        {formatCents(goal.planContributionCents)}
                      </span>
                    </div>
                    <div>
                      ...
                      <span className={styles.goalMetaValue}>
                        {goal.lastContributionDate
                          ? formatShortDate(goal.lastContributionDate)
                          : 'â€”'}
                      </span>
                    </div>
                  </div>
        ...
        <Card className={styles.goalDetailCard}>
          <CardHeader
            ...
            subtitle={
              selectedGoal ? selectedGoal.planSummary : 'Select a goal to review projections.'
            }
              ...
              <div className={styles.goalDetailContent}>
                <div className={styles.goalStatsGrid}>
                  ...
                  <div>
                    ...
                    <span className={styles.goalStatValue}>
                      {formatCents(selectedGoal.planContributionCents)} /{' '}
                  ...
                  <div>
                    ...
                    <span className={styles.goalStatValue}>
                      {formatShortDate(selectedGoal.startDate)} ?{' '}
                      {formatShortDate(selectedGoal.targetDate)}
                    </span>
                  </div>
                </div>
                  ...
                  <ul className={styles.milestoneList}>
                    {upcomingMilestones.map((milestone) => (
                      <li
                        key={milestone.id}
                        className={
                          milestone.achieved ? styles.milestoneComplete : styles.milestonePending
                        }
                      >
                        <div>
                        ...
                        <div className={styles.milestoneMeta}>
                          ...
                          {milestone.achievedAt ? (
                            <span className={styles.milestoneAchieved}>
                              Hit {formatShortDate(milestone.achievedAt)}
                            </span>
                          ) : null}
                        </div>
                      </li>
                  ...
                  <ul className={styles.contributionList}>
                    {sortedContributions.slice(0, 5).map((contribution) => (
                      <li key={contribution.id}>
                        ...
                        {contribution.note ? (
                          <span className={styles.contributionNote}>{contribution.note}</span>
                        ) : null}
                      </li>
                    ))}
                    {!sortedContributions.length ? (
                      <li className={styles.emptyState}>
                        Contributions will appear here once you start funding.
                      </li>
                    ) : null}
                  </ul>
                </section>
                    ...
                    {projection.map((point) => (
                      <div key={point.month} className={styles.projectionItem}>
                        ...
                        <span className={styles.projectionValue}>
                          {formatCents(point.projectedAmountCents)}
                        </span>
                        <span className={styles.projectionTarget}>
                          Target {formatCents(point.targetAmountCents)}
                        </span>
                      </div>
                    ))}
                    {!projection.length ? (
                      <p className={styles.emptyState}>
                        Add projection data to visualize the runway.
                      </p>
                    ) : null}
                  </div>
                </section>
            ...
            <ol className={styles.wizardStepper}>
              {wizardSteps.map((step) => (
                <li
                  ...
                  className={[
                    styles.wizardStep,
                    step.id === wizardStep ? styles.wizardStepActive : '',
                    step.id < wizardStep ? styles.wizardStepComplete : '',
                  ]
                    .filter(Boolean)
                    .join(' ')}
                >
        ...
        <Card className={styles.aiCard}>
          ...
          <CardBody className={styles.aiBody}>
            <div className={styles.promptList}>
              ...
              {!prompts.length ? (
                <p className={styles.emptyState}>
                  Prompts will appear once the assistant has data.
                </p>
              ) : null}
            </div>
        ...
        <CardBody className={styles.insightsBody}>
          {insights.length ? (
            <ul className={styles.insightList}>
              {insights.map((insight) => (
                ...
                >
                  ...
                  {insight.ctaLabel ? (
                    <span className={styles.insightCta}>{insight.ctaLabel}</span>
                  ) : null}
                </li>
              ))}
            </ul>
            ...
            <p className={styles.insightEmpty}>
              The assistant will share ideas here once activity begins.
            </p>
          )}
        </CardBody>
      </Card>
