...
describe('BillsSection', () => {
  ...
  it('orders upcoming reminders chronologically', async () => {
    ...
    expect(reminderItems[1]?.textContent).toContain(
      'Gym membership due today. Mark paid once charged.',
    );
    expect(reminderItems.at(-1)?.textContent).toContain(
      'Power bill will auto-draft this afternoon.',
    );
  });
});
