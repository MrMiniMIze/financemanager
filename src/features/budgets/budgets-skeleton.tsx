    ...
    <section className={styles.section} aria-busy="true" aria-live="polite">
      <Card>
        ...
        >
          ...
          <Skeleton
            className={patterns.skeletonText}
            style={{ width: '24rem', marginTop: '0.4rem' }}
          />
        </CardHeader>
        <CardBody className={styles.cardBody}>
              ...
              <div className={styles.summaryMetrics}>
                {Array.from({ length: 4 }).map((_, index) => (
                  <div key={`budget-summary-metric-${index}`} className={styles.metric}>
                    ...
                    <Skeleton
                      className={patterns.skeletonText}
                      style={{ width: '80%', height: '1.2rem', marginTop: '0.4rem' }}
                    />
                  </div>
                ))}
              </div>
          ...
          <div
            className={`${patterns.grid} ${patterns.gridCols2} ${styles.bottomGrid}`}
            aria-hidden
          >
            <div className={`${patterns.glass} ${styles.alertsPanel}`}>
              <Skeleton
                className={patterns.skeletonText}
                style={{ width: '40%', height: '1.1rem' }}
              />
            ...
            <div className={`${patterns.glass} ${styles.summaryPanel}`}>
              ...
              <div className={styles.summaryMetricsGrid}>
                {Array.from({ length: 4 }).map((_, index) => (
                  <div key={`budget-outlook-${index}`} className={styles.summaryMetric}>
                    ...
                    <Skeleton
                      className={patterns.skeletonText}
                      style={{ width: '75%', height: '1.1rem', marginTop: '0.35rem' }}
                    />
                  </div>
                ))}
              </div>
              ...
              <div className={styles.summaryActions}>
                ...
                <Skeleton
                  className={patterns.skeletonText}
                  style={{ width: '12rem', height: '2.4rem' }}
                />
              </div>
            </div>
          </div>
              ...
              {Array.from({ length: 4 }).map((_, index) => (
                <div key={`budget-table-row-${index}`} className={styles.categoryRow}>
                  <div className={styles.categoryInfo}>
                    ...
                    <Skeleton
                      className={patterns.skeletonText}
                      style={{ width: '35%', marginTop: '0.35rem' }}
                    />
                  </div>
                  <Skeleton
                    className={patterns.skeletonText}
                    style={{ width: '100%', height: '0.65rem' }}
                  />
                </div>
              ))}
            </div>
